...
public class DateUtil {
	...
	public static Date stringDate2(String s) {
		Date date = null;
	 ...
	 * @return
	 */
	public static Date getFirstDayNextMonth(int month) {
		Calendar c = Calendar.getInstance();
		c.setTime(new Date());
	...
	/**
	 * 获取当前月最后一天
	 * @Author xiaopeng@outlook.com
	 * @return
	 */
	public static Date getToMonthLast() {
		Calendar c = Calendar.getInstance();
	 ...
	 * @return
	 */
	public static Long getEndOfMonthTime() {
		Calendar c = Calendar.getInstance();
		Long outTime = c.getTime().getTime();
		//得到一个月最后一天日期
		int MaxDay=c.getActualMaximum(Calendar.DAY_OF_MONTH);
		//按你的要求设置时间
		c.set( c.get(Calendar.YEAR), c.get(Calendar.MONTH), MaxDay, 23, 59, 59);
		outTime = c.getTime().getTime() - outTime;
		return outTime>0?outTime/1000:0l;
	}
...
//		long second1 = minute % 60;
	 ...
	 */
	public static long diffTime(Date date1, Date date2) throws ParseException {
		long time1=timesdf.parse(timesdf.format(date1)).getTime();
		long time2=timesdf.parse(timesdf.format(date2)).getTime();
		return time2/1000/60-time1/1000/60;
	}
	 ...
	 * @throws ParseException
	 */
	public static String getTimeOfDay(Date date1,String pattern) {
	 ...
	 * @return
	 */
	public static Date dateToSystemDateByTiming(Date date) throws ParseException {

		SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyyMMddHHmm");
		String newDate = simpleDateFormat.format(date);
		Date a =  simpleDateFormat.parse(newDate);
		Calendar calendar = Calendar.getInstance();
		calendar.setTime(a);
	 ...
	 * @return
	 */
	public static long dateToSystemDateByTiming(long second) throws ParseException {
		return dateToSystemDateByTiming(new Date(second * 1000)).getTime() /1000;
	}
	 ...
	 * @throws ParseException 
	 */
	public static long startRest(long date, int minute) throws ParseException {
		long countDownTime = 0;
		
		String dateUTC = DateUtil.stringDateUTC(date);
		Date dowmTime = DateUtil.addMinutes(stringDate(dateUTC), minute);
		dowmTime=dateToSystemDateByTiming(dowmTime);
		Date now = new Date();
		if (dowmTime.after(now)) {
			countDownTime = (dowmTime.getTime() - now.getTime()) / 1000;
		}
	  ...
	  */
	   ...
	   * Author:houshenglong
	   * date:2018年5月21日
	   * method:获取时间   当年当月显示:月日时分  往年显示:年月日时分
	   * @parameter
	   */
	  public static String formatDateToNewPhrase(Date date) {
			Calendar now = Calendar.getInstance();
			Calendar target = Calendar.getInstance();
			target.setTime(date);
			if (target.get(Calendar.YEAR) != now.get(Calendar.YEAR)) {
